function Component() {
    ...
    this.showTagName = function(sNamePlace, oObject) {
        console.log('sNamePlace :', sNamePlace);
        console.log('oObject :', oObject);
        $.each(oObject, function(i, fieldName) {
            var oObjectElement = oObject.find(fieldName),
                tagName = oObjectElement.prop('tagName');
            console.log('tagName :', tagName);
            if (tagName == 'IMG') {
                console.log('tagName \'IMG\' :', tagName);
            } else if (tagName == 'INPUT' || tagName == 'TEXTAREA') {
                console.log('tagName INPUT : TEXTAREA', tagName);
            } else { console.log('tagName :', tagName); }
        });
    };
}